...
def ask_vt_key(env):
    ...
    if not sys.stdin.isatty():
        return None
    print("? VirusTotal API key (https://www.virustotal.com/gui/my-apikey): ", end="", flush=True)
    key = input().strip()
    return key or None
...
def build_data(cred, env, vt_key):
    ...
    if t == "telegramApi":
        ...
        data = {"user": env.get("WAZUH_API_USER", ""), "password": env.get("WAZUH_API_PASS", "")}
    elif t == "httpQueryAuth":
        data = {"name": "key", "key": env.get("GSB_API_KEY", "")}
...
def remap_credentials(nodes, name_to_id):
    ...
    for node in nodes:
        ...
        for ctype, ref in list(creds.items()):
            cname = ref.get("name")
            if (cname and cname in name_to_id and name_to_id[cname][0] == ctype
...
def push_credentials(base, key, env, vt_key, dry=False):
    ...
    for cred in CREDENTIALS:
        ...
        if cred["name"] in have:
            ...
            api(base, key, "PUT", f"/api/v1/credentials/{cid}",
                {"name": cred["name"], "type": cred["type"], "data": data})
            print(f"  [update] {cred['name']} (id={cid})")
        else:
            created = api(base, key, "POST", "/api/v1/credentials",
                          {"name": cred["name"], "type": cred["type"], "data": data})
...
def main():
    ...
    ap.add_argument("--vt-key", default="", help="VirusTotal API key (tidak disimpan ke .env)")
    ...
    ap.add_argument("--dry-run", action="store_true", help="tanpa N8N_OWNER_API_KEY: cetak rencana")
    args = ap.parse_args()
